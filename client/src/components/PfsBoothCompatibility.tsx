...
import { ChevronDown, Wind, Fan } from 'lucide-react';
          ...
          <ul className="divide-y divide-white/10">
            {rows.map((row, i) => (
              <li
                ...
                className={`flex items-start gap-3 px-5 py-4 ${i % 2 === 0 ? 'bg-[#141414]' : 'bg-[#101010]'}`}
              >
                <span
                  ...
                  aria-hidden="true"
                >
                  {row.position === 'Intake' ? <Fan className="h-4 w-4" /> : <Wind className="h-4 w-4" />}
                </span>
