  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use("/manus-storage", async (req, res) => {
        ...
        if (!forgeBaseUrl || !forgeKey) { res.writeHead(500); res.end("Not configured"); return; }
        try {
          const forgeUrl = new URL("v1/storage/presign/get", forgeBaseUrl + "/");
          forgeUrl.searchParams.set("path", key);
          const forgeResp = await fetch(forgeUrl, { headers: { Authorization: `Bearer ${forgeKey}` } });
          if (!forgeResp.ok) { res.writeHead(502); res.end("Backend error"); return; }
          const { url } = (await forgeResp.json()) as { url: string };
          if (!url) { res.writeHead(502); res.end("Empty URL"); return; }
...
const isManusEnv = !!process.env.BUILT_IN_FORGE_API_KEY;
const plugins = [
  react(),
  tailwindcss(),
